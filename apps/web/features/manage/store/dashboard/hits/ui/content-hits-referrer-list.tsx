...
import { getContentReferrerStats } from '../api/get-content-referrer-stats';
import { useSearchParams } from 'next/navigation';
import NoContent from '@/shared/ui/NoContent';
import LoadingSpinner from '@/shared/ui/LoadingSpinner';
import Pagination from '@/shared/ui/Pagination';

interface ContentHitsReferrerListProps {
  contentId: string;
}

export default function ContentHitsReferrerList({
  contentId,
}: ContentHitsReferrerListProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['content-hits-referrer-list', contentId, period, page, size],
    queryFn: () => {
      return getContentReferrerStats(contentId, period, page, size);
    },
    select: (res) => ({
      items: res.items ?? [],
