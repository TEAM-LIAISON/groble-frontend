...
import { getMarketHitsContentList } from '../api/get-market-hits';
import { useSearchParams } from 'next/navigation';
import NoContent from '@/shared/ui/NoContent';
import LoadingSpinner from '@/shared/ui/LoadingSpinner';
import Pagination from '@/shared/ui/Pagination';

export default function MarketHitsContentList() {
  const searchParams = useSearchParams();
  const period = searchParams.get('period') ?? 'TODAY';
  const pageFromUrl = Number(searchParams.get('contentPage')) ?? 1;
  const page =
      ...
      <div className="flex flex-col ">
        ...
        {data?.items.length === 0 ? (
          ...
          data?.items.map((item, idx) => (
            <div className="flex justify-between gap-10" key={item.contentId}>
