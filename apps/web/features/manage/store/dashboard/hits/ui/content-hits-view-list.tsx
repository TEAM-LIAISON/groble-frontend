...
interface ContentHitsViewListProps {
  contentId: string;
}

export default function ContentHitsViewList({
  contentId,
}: ContentHitsViewListProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['content-hits-view-list', contentId, period, page, size],
    queryFn: () => {
      return getContentViewStats(contentId, period, page, size);
    },
    select: (res) => ({
      items: res.items ?? [],
